...
import { renderSysDFMEA }              from './pages/safety/dfmea-system.js';
...
route('/project/:projectId/item/:itemId/sw-units', async ({ projectId, itemId }) => {
  setLoading();
  const ctx = await loadItemContext(projectId, itemId, null, 'sw-units');
  if (!ctx) { navigate('/projects'); return; }
...
async function renderSafetyPage(container, ctx, parentType, parentId, analysisType) {
  ...
  } else if (analysisType === 'SYS_DFMEA') {
    await renderSysDFMEA(container, { ...ctx, parentType, parentId });
  } else if (analysisType === 'FTA') {
