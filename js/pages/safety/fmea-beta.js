/**
 * FMEA Beta — full Architecture canvas + FMEA functional dependency layer
 *
 * Base: Architecture Canvas v3 (full copy)
 * Added: FMEA functional dependency edges, propagation engine, DFMEA table
 *
...
// ── FMEA: edge types, inference table, propagation colours ────────────────────
const FMEA_EDGE_TYPES = {
...
const FMEA_PROP_GLOW = { 0:null, 1:'#f59e0b', 2:'#f97316', 3:'#ef4444' };

function fmeaInferEffect(fmText, edgeType) {
  ...
  const k = f.includes('miss')||f.includes('loss')||f.includes('absent') ? 'missing'
    ...
    : 'incorrect';
  return (FMEA_INFER[k]||FMEA_INFER.incorrect)[edgeType]||'Failure propagated to dependent function';
}
  ...
  container.innerHTML = `
    <div class="arch-shell">
      <div class="arch-topbar">
        <span class="arch-topbar-title">⬡ ${escH(title)} — FMEA Beta</span>
        ...
        <div class="arch-canvas-outer" id="arch-outer">
          <div class="arch-viewport" id="arch-vp">
            ...
            <svg class="arch-svg" id="arch-svg" xmlns="http://www.w3.org/2000/svg">
              ...
              <!-- FMEA functional dependency edges -->
              <defs>
                ${Object.entries(FMEA_EDGE_TYPES).map(([k,v]) => `
...
function groupHTML(g) {
  ...
  const funStrip = `
    <div class="arch-group-funs" id="funlist-${g.id}">
      ${funs.map(f => fmeaFunBoxHTML(f, g.id)).join('')}
...
function blockHTML(c) {
  ...
  const funItems = `
    ${funs.map(f => fmeaFunBoxHTML(f, c.id)).join('')}
...
// ═══════════════════════════════════════════════════════════════════════════════
// FMEA Beta — entry point + FMEA-specific functions
// ═══════════════════════════════════════════════════════════════════════════════

export async function renderFmeaBeta(container, ctx) {
  const { project, item, system } = ctx;
  const parentType = ctx.parentType || (system ? 'system' : 'item');
  const parentId   = ctx.parentId   || (system ? system.id : item.id);

  container.innerHTML = '<div class="content-loading"><div class="spinner"></div></div>';

  // Load arch data (same as renderArchitecture but no landing / no sub-pages)
  const [compRes, connRes, sysRes, cfgRes] = await Promise.all([
  ...
  if (compRes.error) {
    container.innerHTML = `<div style="padding:40px;color:var(--color-danger)">
      <strong>Architecture tables not found.</strong><br>Run <code>migration_005_architecture.sql</code> in Supabase.</div>`;
    return;
  }
  ...
  let funs = [];
  if (compList.length) {
    const { data } = await sb.from('arch_functions').select('*').in('component_id', compList.map(c => c.id)).order('sort_order');
    funs = data || [];
  }

  // Sub-components + failure modes (same as architecture.js)
  const allCompIds = compList.map(c => c.id);
  let scByBlockId = {};
  if (allCompIds.length) {
    const { data: allScs } = await sb.from('sub_components').select('*').in('block_id', allCompIds).order('sort_order');
    (allScs||[]).forEach(sc => { sc._fms = []; (scByBlockId[sc.block_id] ||= []).push(sc); });
  }
  const allFnIds = funs.map(f => f.id);
  const allScIds = Object.values(scByBlockId).flat().map(sc => sc.id);
  let fnFmsByFnId = {}, fnFmsByCompId = {}, fnFmsByScId = {};
  if (allFnIds.length || allCompIds.length || allScIds.length) {
    ...
    if (allCompIds.length) parts.push(`component_id.in.(${allCompIds.join(',')})`);
    if (allScIds.length)   parts.push(`sub_component_id.in.(${allScIds.join(',')})`);
    const { data: allFms } = await sb.from('arch_function_fms').select('*').or(parts.join(',')).order('sort_order');
    (allFms||[]).forEach(fm => {
      if (fm.function_id)       (fnFmsByFnId[fm.function_id]  ||= []).push(fm);
      else if (fm.sub_component_id) (fnFmsByScId[fm.sub_component_id] ||= []).push(fm);
      else if (fm.component_id) (fnFmsByCompId[fm.component_id] ||= []).push(fm);
    });
  }
  funs.forEach(f => { f._fms = fnFmsByFnId[f.id] || []; });
  Object.values(scByBlockId).flat().forEach(sc => { sc._fms = fnFmsByScId[sc.id] || []; });
  ...
  // Load FMEA data
  const [fmeaEdgeRes, fmeaCfgRes, fmeaRuleRes] = await Promise.all([
  ...
  _s.fmeaCfgMap = {};
  (fmeaCfgRes.data || []).forEach(c => { _s.fmeaCfgMap[c.fn_id] = c; });

  const rulesByEdge = {};
  (fmeaRuleRes.data || []).forEach(r => { (rulesByEdge[r.edge_id] ||= []).push(r); });

  const allFnIdSet = new Set(_s.components.flatMap(c => (c.functions||[]).map(f => f.id)));
  _s.fmeaEdges = (fmeaEdgeRes.data || [])
    .filter(e => allFnIdSet.has(e.source_fn_id) && allFnIdSet.has(e.target_fn_id))
    .map(e => ({ ...e, _rules: rulesByEdge[e.id] || [] }));

  if (window._archCleanup) window._archCleanup();
  buildShell(container, system ? system.name : (item?.name || 'FMEA'));
  renderAll();
  wireCanvas();
  wireGlobal();
  wireFmeaLayer();
...
function fmeaFunBoxHTML(f, compId) {
  const prop  = _s?.fmeaPropagated?.[f.id];
  const isInj = _s?.fmeaInjected?.fnId === f.id;
  const glowColor = isInj ? '#c45f00' : prop?.cut ? '#1a8f5c' : FMEA_PROP_GLOW[prop?.level];
  ...
  return `<div class="arch-fun-box ${f.is_safety_related ? 'arch-fun-box--safe' : ''}"
       data-fun-id="${f.id}" data-comp-id="${compId}" data-fun-ref-id="${f.function_ref_id||''}"${funTooltipAttrs(f)}
       style="position:relative;${borderStyle}${bgStyle}">
    ...
    <button class="arch-fun-del" data-fun-id="${f.id}" data-comp-id="${compId}" title="Remove">✕</button>
    <span class="fmea-fn-handle" data-fn-id="${f.id}"
      ...
      box-shadow:0 1px 3px rgba(0,0,0,.3);z-index:10;display:none;"></span>
  </div>`;
}

// ── FMEA: show/hide handles on hover ──────────────────────────────────────────
function wireFmeaHandles() {
  document.querySelectorAll('.arch-fun-box').forEach(box => {
    const h = box.querySelector('.fmea-fn-handle');
    if (!h) return;
    h.style.display = '';
...
function wireFmeaToolbar() {
  ...
  document.getElementById('fb-clear-btn')?.addEventListener('click', fmeaClearInjection);
...
// ── FMEA: apply propagation glow to function boxes ────────────────────────────
function applyFmeaGlow() {
  document.querySelectorAll('.arch-fun-box[data-fun-id]').forEach(el => {
    const fnId = el.dataset.funId;
    const prop  = _s.fmeaPropagated[fnId];
    const isInj = _s.fmeaInjected?.fnId === fnId;
    if (isInj) {
      el.style.borderColor = '#c45f00'; el.style.boxShadow = '0 0 6px #c45f0044'; el.style.background = '#fff3e6';
    } else if (prop?.cut) {
      el.style.borderColor = '#1a8f5c'; el.style.boxShadow = '0 0 6px #1a8f5c44'; el.style.background = '#e6f6ee';
    } else if (prop?.level === 3) {
      el.style.borderColor = '#ef4444'; el.style.boxShadow = '0 0 6px #ef444444'; el.style.background = '#fde8ed';
    } else if (prop?.level) {
      el.style.borderColor = '#f97316'; el.style.boxShadow = '0 0 6px #f9731644'; el.style.background = '#fff8e6';
    } else {
...
// ── FMEA: render functional dependency edges ──────────────────────────────────
function renderFmeaEdges() {
  const g = document.getElementById('fmea-edge-g');
  if (!g || !_s?.fmeaMode) return;

  const allFns = _s.components.flatMap(c => c.functions || []);
  const outer  = document.getElementById('arch-outer');
  if (!outer) return;

  g.innerHTML = _s.fmeaEdges.map(e => {
    const srcFn = allFns.find(f => f.id === e.source_fn_id);
    const tgtFn = allFns.find(f => f.id === e.target_fn_id);
    if (!srcFn || !tgtFn) return '';

    const srcEl = document.querySelector(`.arch-fun-box[data-fun-id="${srcFn.id}"]`);
    const tgtEl = document.querySelector(`.arch-fun-box[data-fun-id="${tgtFn.id}"]`);
    let sx, sy, tx, ty;
    if (srcEl && tgtEl) {
      const sr = srcEl.getBoundingClientRect();
      const tr = tgtEl.getBoundingClientRect();
      const vp = outer.getBoundingClientRect();
      const z  = _s.zoom;
      ...
      const srcComp = _s.components.find(c => c.id === srcFn.component_id);
      const tgtComp = _s.components.find(c => c.id === tgtFn.component_id);
    ...
    const prop    = _s.fmeaPropagated[tgtFn.id];
    const propSrc = _s.fmeaPropagated[srcFn.id] || _s.fmeaInjected?.fnId === srcFn.id;
    const isActive = !!(propSrc && prop);
    const isCut    = prop?.cut;
    const isCrit   = prop?.level === 3;
    const isSel    = _s.fmeaSelectedEdgeId === e.id;
    ...
    const marker = `fmea-arr-${e.edge_type}`;
    const cx1 = sx + Math.abs(tx-sx)*0.45, cx2 = tx - Math.abs(tx-sx)*0.45;
    ...
    return `<g class="fmea-edge-hit" data-eid="${e.id}" style="cursor:pointer;pointer-events:stroke;">
      ...
      ${(e.label || isSel) ? `
        <rect x="${mx-28}" y="${my-9}" width="56" height="17" rx="3" fill="white" stroke="${color}" stroke-width="1" opacity=".95"/>
      ...
      ${e.diagnostic_coverage > 0 ? `
        <text x="${mx}" y="${my+20}" text-anchor="middle" font-size="9" font-family="system-ui" fill="#1a8f5c">Diag ${e.diagnostic_coverage}%</text>` : ''}
    </g>`;
  }).join('');
  ...
  g.querySelectorAll('.fmea-edge-hit').forEach(el => {
    el.addEventListener('click', ev => {
      ev.stopPropagation();
      _s.fmeaSelectedEdgeId = el.dataset.eid;
      fmeaShowEdgeProps(_s.fmeaEdges.find(x => x.id === el.dataset.eid));
      renderFmeaEdges();
      fmeaSwitchTab('props');
    });
  });
...
// ── FMEA: draw new edges by dragging the ● handle ────────────────────────────
function wireFmeaLayer() {
  ...
  document.addEventListener('pointerdown', e => {
    ...
    const onMove = mv => {
      ...
      if (dragG) dragG.innerHTML = `<line
        x1="${(srcX-_s.panX)/_s.zoom}" y1="${(srcY-_s.panY)/_s.zoom}"
    ...
    const onUp = async mv => {
      document.removeEventListener('pointermove', onMove);
      document.removeEventListener('pointerup',   onUp);
      if (dragG) dragG.innerHTML = '';
      const tgtEl = document.elementFromPoint(mv.clientX, mv.clientY)?.closest('.arch-fun-box');
      const toId  = tgtEl?.dataset.funId;
      if (toId && toId !== fromId) {
        const exists = _s.fmeaEdges.find(x => x.source_fn_id === fromId && x.target_fn_id === toId);
        if (!exists) await fmeaCreateEdge(fromId, toId);
      }
    };
    document.addEventListener('pointermove', onMove);
    document.addEventListener('pointerup',   onUp);
...
async function fmeaCreateEdge(fromId, toId) {
  const allFns = _s.components.flatMap(c => c.functions || []);
  const fromFn = allFns.find(f => f.id === fromId);
  ...
  for (const fm of fromFn?._fms || []) {
    const { data: r } = await sb.from('fmea_propagation_rules').insert({
      ...
      effect_at_target: fmeaInferEffect(fm.failure_mode, 'depends_on'),
...
// ── FMEA: inject failure + BFS propagation ────────────────────────────────────
function fmeaInject(fnId, fmId) {
  const allFns  = _s.components.flatMap(c => c.functions || []);
  const startFm = allFns.find(f => f.id === fnId)?._fms?.find(f => f.id === fmId);
  _s.fmeaInjected   = { fnId, fmId };
  _s.fmeaPropagated = {};
  const queue   = [{ id: fnId, level: 0, path: [fnId] }];
  const visited = new Set([fnId]);
  _s.fmeaPropagated[fnId] = { level: 0, path: [fnId], effects: [] };
  const activeFmIds = new Set([fmId]);

  while (queue.length) {
    const { id, level, path } = queue.shift();
    _s.fmeaEdges.forEach(e => {
      ...
      if (e.diagnostic_coverage === 100) {
        visited.add(nextId);
        _s.fmeaPropagated[nextId] = { level: 0, cut: true, cutReason: 'Diagnostic 100%', path: [...path, nextId], effects: [] };
        return;
      }
      const matchRules = (e._rules||[]).filter(r => activeFmIds.has(r.source_fm_id));
      const effects    = matchRules.length ? matchRules.map(r => r.effect_at_target) : startFm ? [fmeaInferEffect(startFm.failure_mode, e.edge_type)] : ['Failure propagated'];
      ...
      const tgtFn  = allFns.find(f => f.id === nextId);
      const tgtCfg = _s.fmeaCfgMap[nextId];
      ...
      if (cut) { visited.add(nextId); _s.fmeaPropagated[nextId] = { level: 0, cut: true, cutReason, path: [...path, nextId], effects }; return; }
      ...
      visited.add(nextId);
      _s.fmeaPropagated[nextId] = { level: newLevel, path: [...path, nextId], effects };
      queue.push({ id: nextId, level: level+1, path: [...path, nextId] });
      (tgtFn?._fms||[]).forEach(f => activeFmIds.add(f.id));
    });
  }

  const affected = Object.keys(_s.fmeaPropagated).length - 1;
  const critical = Object.values(_s.fmeaPropagated).filter(p => p.level === 3).length;
  const cut      = Object.values(_s.fmeaPropagated).filter(p => p.cut).length;
  const status   = document.getElementById('fb-prop-status');
  const clearBtn = document.getElementById('fb-clear-btn');
  if (status)   status.textContent   = `⚡ ${affected} affected${critical ? ` · ⚠ ${critical} critical` : ''}${cut ? ` · ✓ ${cut} cut` : ''}`;
  if (clearBtn) clearBtn.style.display = '';
  renderAll();
}

function fmeaClearInjection() {
  _s.fmeaInjected   = null;
  _s.fmeaPropagated = {};
  const status   = document.getElementById('fb-prop-status');
  const clearBtn = document.getElementById('fb-clear-btn');
...
// ── FMEA: add inject buttons to the function props section ────────────────────
// Called from wireProps after the existing function props are shown
function fmeaWireFnInjectBtns() {
  document.querySelectorAll('.fmea-inject-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      e.stopPropagation();
...
// Called from openProps when a function or component is selected
function fmeaAppendFnSection(fnId) {
  const allFns = _s.components.flatMap(c => c.functions || []);
  const fn = allFns.find(f => f.id === fnId); if (!fn) return;
  const body = document.getElementById('arch-props-body'); if (!body) return;
  const cfg  = _s.fmeaCfgMap[fnId];
  const prop = _s.fmeaPropagated[fnId];
  const isInj = _s.fmeaInjected?.fnId === fnId;

  const propBadge = isInj ? fmeaBadge('⚡ INJECTION POINT','#c45f00')
    : prop?.cut     ? fmeaBadge('✓ '+prop.cutReason, '#1a8f5c')
    : prop?.level===3 ? fmeaBadge('⚠ CRITICAL EFFECT','#ef4444')
    : prop?.level===2 ? fmeaBadge('⚡ PROPAGATED','#f97316')
    : prop?.level===1 ? fmeaBadge('→ LOCAL EFFECT','#f59e0b') : '';

  const fmRows = (fn._fms||[]).map(fm => `
    <div style="padding:5px 8px;border-radius:4px;background:var(--bg-hover);border:1px solid var(--color-border);margin-bottom:4px;">
      <div style="display:flex;align-items:center;gap:6px;">
        <span style="color:var(--color-danger);font-size:11px;">⚠</span>
        <span style="font-size:12px;flex:1;">${escH(fm.failure_mode)}</span>
        <button class="fmea-inject-btn btn-sm" data-fnid="${fn.id}" data-fmid="${fm.id}"
  ...
  section.innerHTML = `
    ...
    <div style="margin-bottom:8px;">
      <div class="arch-props-section-label">Failure Modes — click ⚡ to inject</div>
      ${fmRows || '<div style="font-size:11px;color:var(--color-text-muted);">No FMs defined for this function.</div>'}
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:5px;margin-bottom:6px;">
      <label style="display:flex;align-items:center;gap:8px;font-size:12px;cursor:pointer;">
        <input type="checkbox" id="fb-has-red" ${cfg?.has_redundancy?'checked':''}> Redundancy
      </label>
      ...
      <label style="display:flex;align-items:center;gap:8px;font-size:12px;cursor:pointer;">
        <input type="checkbox" id="fb-has-ss" ${cfg?.has_safe_state?'checked':''}> Safe state
      </label>
  ...
  document.getElementById('fb-has-red')?.addEventListener('change', () => { section.remove(); fmeaAppendFnSection(fnId); });
  document.getElementById('fb-has-ss')?.addEventListener('change',  () => { section.remove(); fmeaAppendFnSection(fnId); });
  document.getElementById('fb-save-cfg')?.addEventListener('click', async () => {
    const payload = {
      fn_id: fnId, project_id: _s.project.id,
      parent_type: _s.parentType, parent_id: _s.parentId,
    ...
    if (data) _s.fmeaCfgMap[fnId] = data;
    section.remove(); fmeaAppendFnSection(fnId);
    renderAll();
  });
...
function fmeaShowEdgeProps(edge) {
  if (!edge) return;
  const allFns = _s.components.flatMap(c => c.functions || []);
  const srcFn  = allFns.find(f => f.id === edge.source_fn_id);
  const tgtFn  = allFns.find(f => f.id === edge.target_fn_id);
  const et     = FMEA_EDGE_TYPES[edge.edge_type] || FMEA_EDGE_TYPES.depends_on;
  const rules  = edge._rules || [];
  const srcFms = srcFn?._fms || [];

  showPropsPanel(`
    <div class="arch-props-hdr" style="font-size:13px;font-weight:600;margin-bottom:10px;">
      ⬡ FMEA Connection<br>
      <span style="font-size:11px;font-weight:400;color:var(--color-text-muted);">
        ${escH(srcFn?.name||'?')} <span style="color:${et.color};">→</span> ${escH(tgtFn?.name||'?')}
      </span>
    </div>

    <div class="arch-props-section-label">Edge type</div>
    <select id="fb-etype" class="form-select" style="width:100%;margin-bottom:8px;">
      ${Object.entries(FMEA_EDGE_TYPES).map(([k,v])=>`<option value="${k}" ${k===edge.edge_type?'selected':''}>${v.label}</option>`).join('')}
    </select>

    <div class="arch-props-section-label">Signal / Label</div>
    <input id="fb-elabel" class="form-input" value="${escH(edge.label||'')}" placeholder="e.g. CAN signal, PWM…" style="width:100%;margin-bottom:8px;">
    ...
    <input id="fb-diag-mech" class="form-input" value="${escH(edge.diagnostic_mechanism||'')}" placeholder="e.g. CRC, watchdog…" style="width:100%;margin-bottom:8px;">
    <button id="fb-save-edge" class="btn btn-primary btn-sm" style="width:100%;margin-bottom:12px;">Save connection</button>

    <div class="arch-props-section-label">Propagation Rules</div>
    <div id="fb-rules-list">
      ${rules.map(r => {
        ...
        return `<div style="padding:5px 8px;border-radius:4px;background:var(--bg-hover);border:1px solid var(--color-border);margin-bottom:4px;font-size:11px;">
          ...
          <div style="display:flex;align-items:center;gap:6px;margin-top:3px;">
            <span style="font-size:10px;color:var(--color-text-muted);">Sev ${r.severity}/10${r.is_inferred?' (inferred)':''}</span>
            <button class="fb-del-rule btn-sm" data-rid="${r.id}" style="margin-left:auto;font-size:10px;color:var(--color-danger);">✕</button>
          </div>
        </div>`;
      }).join('') || `<div style="font-size:11px;color:var(--color-text-muted);">${srcFms.length?'No rules yet.':'Add FMs to source function first.'}</div>`}
    </div>
    ${srcFms.length ? `<button id="fb-add-rule" class="btn btn-sm" style="width:100%;margin-top:4px;margin-bottom:10px;">＋ Add rule</button>` : ''}
  ...
  document.getElementById('fb-add-rule')?.addEventListener('click', () => fmeaShowRuleForm(edge, srcFms, null));
  ...
  document.getElementById('fb-del-edge')?.addEventListener('click', async () => {
    ...
    _s.fmeaEdges = _s.fmeaEdges.filter(e => e.id !== edge.id);
    _s.fmeaSelectedEdgeId = null;
    showPropsEmpty(); renderAll();
  });
}

function fmeaShowRuleForm(edge, srcFms, existingRule) {
  const list = document.getElementById('fb-rules-list'); if (!list) return;
  list.insertAdjacentHTML('beforeend', `
    <div id="fb-rule-form" style="padding:8px;background:var(--bg-hover);border:1px solid var(--color-border);border-radius:5px;margin-top:4px;">
  ...
  document.getElementById('fb-add-rule')?.style && (document.getElementById('fb-add-rule').style.display = 'none');
  document.getElementById('fb-rf-fm')?.addEventListener('change', ev => {
    const fm  = srcFms.find(f => f.id === ev.target.value);
    const eff = document.getElementById('fb-rf-eff');
    if (eff && fm && !existingRule) eff.value = fmeaInferEffect(fm.failure_mode, edge.edge_type);
  });
...
// ── FMEA: DFMEA table ─────────────────────────────────────────────────────────
function fmeaGenerateTable() {
  const allFns = _s.components.flatMap(c => c.functions || []);
  const rows = [];
  allFns.forEach(fn => {
    const comp = _s.components.find(c => c.id === fn.component_id);
    (fn._fms||[]).forEach(fm => {
      const outEdges = _s.fmeaEdges.filter(e => e.source_fn_id === fn.id);
      const rules    = outEdges.flatMap(e => (e._rules||[]).filter(r => r.source_fm_id === fm.id));
      const effects  = rules.length ? rules.map(r=>r.effect_at_target) : outEdges.map(e=>fmeaInferEffect(fm.failure_mode, e.edge_type));
      const nextFns  = [...new Set(outEdges.map(e=>allFns.find(f=>f.id===e.target_fn_id)?.name).filter(Boolean))];
      const endFns   = outEdges.map(e=>allFns.find(f=>f.id===e.target_fn_id)).filter(f=>f?.is_safety_related).map(f=>f.name);
      const maxSev   = rules.length ? Math.max(...rules.map(r=>r.severity)) : outEdges.length ? 5 : null;
      const cutCount = outEdges.filter(e => e.diagnostic_coverage===100 || _s.fmeaCfgMap[e.target_fn_id]?.has_redundancy || _s.fmeaCfgMap[e.target_fn_id]?.fault_tolerance==='full').length;
  ...
  const el = document.getElementById('fb-table-body'); if (!el) return;
  if (!rows.length) { el.innerHTML = '<div style="padding:16px;color:var(--color-text-muted);">No failure modes found. Add FMs to functions in the Properties panel.</div>'; return; }
  ...
  el.innerHTML = `
    <table style="width:100%;border-collapse:collapse;">
      <thead><tr style="background:var(--bg-hover);">
        ...
        <th style="${thS}">Propagated</th><th style="${thS}">End Effect</th>
      ...
      <tbody>
        ${rows.map((r,i) => `<tr style="background:${i%2===0?'var(--bg-card)':'var(--bg-hover)'};">
          ...
          <td style="${tdS}">${escH(r.fn)}${r.safety?` <span style="color:var(--color-danger);font-size:9px;font-weight:700;">FS</span>`:''}${r.isInferred?` <span style="font-size:9px;color:var(--color-text-muted);" title="Inferred">~</span>`:''}</td>
...
function fmeaBadge(text, color) {
  return `<span style="display:inline-block;background:${color}18;border:1px solid ${color}55;color:${color};padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;">${text}</span>`;
}
